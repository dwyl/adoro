...
h.getPosts = function(callback) {
  ...
  // sets up the posts -> goes and gets title, first few lines of post and creates a url for the post.
  mds.map(function(file){
    var filepath = path.resolve('./posts/'+file);
    fs.stat(filepath, function(err, stats){
      errors.push(err);
      // console.log("helpers >>>>>>>>>>> " + stats);
      if(!err) {
        ...
        fs.readFile(filepath, 'utf8', function(error, data){
          ...
          console.log('POSTS!!!!!!! >>>>>>>>>' + post.full);
          posts.push(post);
          if(--remaining === 0){
            // sorts posts by date decending (newest first)
...
h.convertImgTagInPost = function (post) {
  ...
  // console.log('outside function convertImgTagInPost', post);
// tbc
